const {
	sanitize,
	handle_result,
	handle_fail,
	isAdmin }												= require('../utils');
const router											= require('express').Router();
const db													= require('../db');
const v														= require('../validation');
const auth												= require('../auth');
const { check, validationResult } = require('express-validator/check');

// Config file
const constant = require('../constant');

// Bring Post Model
const Comment = require('../Models/comment');

const commentValidation = [
	check('content').isLength({min: 10}).withMessage('طول متن باید حداقل 10 حرف باشد'),
	check('fullname').isLength({min: 10, max:32}).withMessage('نام و نام خانوادگی را به صورت صحیح وارد کنید'),
	check('email').isEmail({max:32}).withMessage('ایمیل خود را به صورت صحیح وارد کنید'),
];

// Show comments
router.get('/:postId/comments', (req, res) => {
	db.getClient((err, client, done) =>{
		Comment.get(client, req.params.postId)
			.then(rows => {
				return handle_result(res, 'comments', rows, rows);
			})
			.catch(err => handle_fail(res, err))
			.then(() => client.release());
	});
});

// Add comment
router.post('/:postId/comments', commentValidation, (req, res) => {
	// Check for errors
	const errors = validationResult(req);
	if(!errors.isEmpty()){
		return handle_fail(res, errors.array().map(obj => obj.msg).join("\n"), 422);
	}
	
	// Variables
	let parentId = req.body.parent_id || null;
	let fields = {
		content		: req.body.content,
		email			: req.body.email,
		fullname	: req.body.fullname,
		post_id		: req.params.postId,
		parent_id : parentId
	};

	db.getClient((err, client, done) => {
		Comment.add(client, req.params.postId, fields)
			.then(isAdded => {
				if(isAdded !== true){
					throw new Error(isAdded);
				}else{
					return handle_result(res, null, isAdded, null);
				}
			})
			.catch(err => handle_fail(res, err.message))
			.then(() => client.release());
	});
});

// Edit comment
/*router.put('/:postId/comments/:commentId', auth.authenticate, commentValidation, (req, res) => {
	// Check for errors
	const errors = validationResult(req);
	if(!errors.isEmpty()){
		return handle_fail(res, errors.array().map(obj => obj.msg).join("\n"), 422);
	}
	
	let fields		= {};
	let where			= {};
	let commentId = req.params.commentId;
	let postId		= req.params.postId;
	let user			= req.user;

	if(isAdmin(user.role)){
		let adminId = user.id;
		fields = {
			content  : req.body.content,
			status	 : req.body.status || constant.status.ACCEPTED
		};
		where  = {
			id			 : commentId,
		};
	}else{
		fields = {
			content  : req.body.content,
		};
		where  = {
			id			 : commentId,
			user_id  : user.id,
			status	 : constant.status.PENDING
		};
	}

	db.getClient((err, client, done) => {
		Comment.update(client, fields, where)
			.then(isEdited => {
				if(isEdited){
					return handle_result(res, null, isEdited, null);
				}else{
					throw new Error(isEdited);
				}
			})
			.catch(err => handle_fail(res, err.message))
			.then(() => client.release());
	});
});

// Delete comment
router.delete('/:postId/comments/:commentId', auth.authenticate, (req, res) => {
	// Check for errors
	const errors = validationResult(req);
	if(!errors.isEmpty()){
		return handle_fail(res, errors.array().map(obj => obj.msg).join("\n"), 422);
	}

	let where			= {};
	let user			= req.user;
	let commentId = req.params.commentId;

	if(isAdmin(user.role)){
		where  = {
			id : commentId,
		};
	}else{
		where  = {
			id				: commentId,
			author_id : user.id,
			status		: constant.status.PENDING
		};
	}
	db.getClient((err, client, done) => {
		Comment.delete(client, where)
			.then(isDeleted => {
				if(!isDeleted)
					throw new Error('چنین نظری وجود ندارد');
				return handle_result(res, null, isDeleted, null);
			})
			.catch(err => handle_fail(res, err.message))
			.then(() => client.release());
	});
});*/


module.exports = router;
